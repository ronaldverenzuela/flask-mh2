...
def balance():
    ...
    total_activos=total_mp+total_aba+total_fpc+total_a_fpc
    total_pasivos=total_fpp+total_a_fpp+total_gastos
...
def ordenes_produccion():
    ...
    return render_template('ordenes_produccion.html',array_data=array_data)
...
def crear_formula():
    ...
    if request.method=='POST':
        ...
        for j in array_productos:
            ...
            array_data.append(request.form[array_variable[q]])
            q=q+1
...
def planta_aba():
    ...
    cur=mysql.connection.cursor()
    cur.execute(f'SELECT * FROM formula WHERE alimento = "PONEDORA" ')
    #cur.execute("CALL ver_precio(1)")
    ...
    costo_p=[]
    costo_p.append(calcular_costo_produccion())
    print(costo_p)
    costo_p_final=[]
...
def delete_inventario_general(id,c_bd,k_bd,producto):


    print('los datos son id = ',id, ' k_bd = ',k_bd, ' c_bd = ',c_bd,' producto= ',producto)
...
if __name__ == '__main__':
    ...
    #app.run(host='192.168.103.237', port=port, debug=True)
